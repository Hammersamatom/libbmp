...
BmpPixbuf::BmpPixbuf(const int32_t width, const int32_t height)
{
	init(width, height);
}
...
void BmpPixbuf::init(const int32_t width, const int32_t height)
{
...
void BmpPixbuf::set_pixel(const int32_t x, const int32_t y, const uint8_t r, const uint8_t g, const uint8_t b)
{
...
uint8_t BmpPixbuf::red_at(const int32_t x, const int32_t y)
{
	return data[(x * len_pixel) + (y * len_row) + 2];
}

uint8_t BmpPixbuf::green_at(const int32_t x, const int32_t y)
{
	return data[(x * len_pixel) + (y * len_row) + 1];
}

uint8_t BmpPixbuf::blue_at(const int32_t x, const int32_t y)
{
	return data[(x * len_pixel) + (y * len_row)];
}

void BmpPixbuf::write(const int32_t row, std::ofstream& f)
{
	f.write(reinterpret_cast<char*> (&data[row * len_row]), len_row);
}

void BmpPixbuf::read(const int32_t row, std::ifstream& f)
{
...
BmpImg::BmpImg(const int32_t width, const int32_t height) : BmpPixbuf (width, std::abs (height))
{
...
int32_t BmpImg::get_width(void)
{
	return header.biWidth;
}

int32_t BmpImg::get_height(void)
{
